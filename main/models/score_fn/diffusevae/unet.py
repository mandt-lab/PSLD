...
@register_module(category="score_fn", name="diffvae")
class UNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config.model
        ch, out_ch, ch_mult = (
            self.config.score_fn.model_channels,
            self.config.score_fn.out_channels,
            tuple(self.config.score_fn.channel_mult),
        )
        attn_resolutions = self.config.score_fn.attention_resolutions
        resamp_with_conv = self.config.score_fn.conv_resample

        self.ch = ch
        self.temb_ch = self.ch * 4
        self.num_resolutions = len(ch_mult)
        self.num_res_blocks = self.config.score_fn.num_res_blocks
        self.resolution = config.data.image_size
        self.in_channels = self.config.score_fn.in_channels
        self.num_heads = self.config.score_fn.num_heads
        self.num_heads_upsample = (
            self.num_heads
            if self.config.score_fn.num_heads_upsample == -1
            else self.config.score_fn.num_heads_upsample
        )
        self.dropout = self.config.score_fn.dropout
        ...
        self.proj = None
        if self.config.score_fn.use_z:
            self.proj = nn.Sequential(
                nn.Linear(self.config.score_fn.z_dim, self.temb_ch),
                nn.SiLU(),
                nn.Linear(self.temb_ch, self.temb_ch),
            )

        self.num_classes = self.config.score_fn.num_classes

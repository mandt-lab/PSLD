...
@register_module(category="score_fn", name="ncsnpp_cld")
class NCSNpp(nn.Module):
    def __init__(self, config):
        ...
        self.n_channels = self.config.score_fn.nf
        ch_mult = self.config.score_fn.ch_mult
        self.n_resblocks = self.config.score_fn.num_res_blocks
        self.attn_resolutions = self.config.score_fn.attn_resolutions
        dropout = self.config.score_fn.dropout
        resamp_with_conv = self.config.score_fn.resamp_with_conv
        self.num_resolutions = len(ch_mult)
        self.all_resolutions = [config.data.image_size //
                                (2 ** i) for i in range(self.num_resolutions)]

        fir = self.config.score_fn.fir
        fir_kernel = self.config.score_fn.fir_kernel
        self.skip_rescale = self.config.score_fn.skip_rescale
        self.resblock_type = self.config.score_fn.resblock_type.lower()
        self.progressive = self.config.score_fn.progressive.lower()
        self.progressive_input = self.config.score_fn.progressive_input.lower()
        self.embedding_type = self.config.score_fn.embedding_type.lower()
